...
function! s:ToggleWord()
    ...
    let words_candicates_array = []

    " Load user specified toggle sets first
    if exists('g:toggle_words_dict')
        " File specific first
        if has_key(g:toggle_words_dict, cur_filetype)
            call extend(words_candicates_array, g:toggle_words_dict[cur_filetype])
        endif
        if has_key(g:toggle_words_dict, '*')
            call extend(words_candicates_array, g:toggle_words_dict['*'])
        endif
    endif

    " Then add default candidates
    call extend(words_candicates_array, g:default_toggle_words_dict['*'])
    if has_key(g:default_toggle_words_dict, cur_filetype)
        call extend(words_candicates_array, g:default_toggle_words_dict[cur_filetype])
    endif

    let cur_word = expand("<cword>")
    let word_attr = 0 " 0 - eXaCt; 1 - Capital; 2 - UPPERCASE; 3 - lowercase

    if tolower(cur_word) ==# cur_word
        let word_attr = 3
    elseif toupper(cur_word) ==# cur_word
        let word_attr = 2
